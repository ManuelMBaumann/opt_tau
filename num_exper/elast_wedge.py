...
from mskrylov import pmsgmres, pmsgmres_experm, test_orig_problem
import time
from math import pi
...
def main( ndims=2,          # problem dimension (2,3) 
          ...
          Nom = 5,
          plots=False,       # plot of parameters and num. solution
          spy=False,        # provide spy plot of matrices
  ...
  if storing:
    ...
    mmwrite('matrix_io/M.mtx', M.toscipy())
      ...
      if dg_pp < 0:
          print('Use pythons sparse direct solver...')
          ...
          print('Use pmsGMRES solver')
          t0 = time.time()
          #sol, it = pmsgmres(K.toscipy().tocsc(), C.toscipy().tocsc(), M.toscipy().tocsc(), rhs, freq, tau_re+1j*tau_im, damping, tol, maxit, dg_pp)
          sol, it = pmsgmres_experm(K.toscipy().tocsc(), C.toscipy().tocsc(), M.toscipy().tocsc(), rhs, freq, tol, maxit, dg_pp)
          te = time.time()
